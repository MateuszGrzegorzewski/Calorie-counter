...
class ProductsToDailyMealsModel(db.Model):
    ...
    def __init__(self, date_of_meal, name_of_meal, product_id, weight):
    ...
    @classmethod
    def find_by_date_and_mealtime(cls, date, mealtime):
        return cls.query.filter_by(date_of_meal=date).filter_by(name_of_meal=mealtime).all()

    @classmethod
    def find_by_product(cls, date, mealtime, product_id):
        return cls.query.filter_by(date_of_meal=date).filter_by(name_of_meal=mealtime).filter_by(product_id=product_id).first()
